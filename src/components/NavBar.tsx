...
const navItems: NavItem[] = [
  ...
  { number: "03", label: "Projects", id: "projects", href: "#projects" },
        ...
        <div className="hidden items-center gap-10 md:flex">
          {navItems.map((item) => (
            <a
              ...
              className="group inline-flex items-center text-mono-caps text-secondary transition-all duration-200 hover:scale-105"
            >
              <span
                className={`text-mono-caps-sm transition-colors duration-200 ${activeSection === item.id
                    ? "text-accent"
                    : "text-secondary group-hover:text-(--color-accent)"
                  }`}
              >
                {item.number}/
              </span>
